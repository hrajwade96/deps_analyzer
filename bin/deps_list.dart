...
Future<void> findDependencies(
  ...
  final rootPubspecPath = path.join(currentDir, 'pubspec.yaml');
  final glob = Glob(path.join(currentDir, '**/pubspec.yaml'));

  final allDependencies = <String, Set<String>>{};

  // Process the root package's pubspec.yaml file first
  if (File(rootPubspecPath).existsSync() &&
      !shouldIgnore(rootPubspecPath, paths)) {
    await _processPubspec(File(rootPubspecPath), packages, allDependencies);
  }

  // Process other pubspec.yaml files in subdirectories
  await for (final FileSystemEntity entity in glob.list(followLinks: false)) {
    ...
    if (entity is File && !shouldIgnore(normalizedFilePath, paths)) {
      await _processPubspec(entity, packages, allDependencies);
    }
  }
...
/// Helper function to process a pubspec.yaml file and extract dependencies.
Future<void> _processPubspec(File pubspecFile, List<String> packages,
    Map<String, Set<String>> allDependencies) async {
  try {
    ...
    if (doc is YamlMap && doc.containsKey('dependencies')) {
      ...
      if (dependencies is YamlMap) {
        dependencies.forEach((key, value) {
          ...
          var versionInfo = projectPackage;
          if (value == null) {
            ...
            if (value.containsKey('version')) {
              ...
              versionInfo = value.keys.map((k) => '$k: ${value[k]}').join(', ');
            }
          }
          ...
          if (checkList.contains(packageName)) {
            safePrint('Skipping dependency $packageName');
            return; // Skip dependency
          }

          if (versionInfo.contains('{path:')) {
            return;
          }

          allDependencies.putIfAbsent(packageName, () => <String>{});
          allDependencies[packageName]!.add(versionInfo);
          safePrint('Found dependency: $packageName, version: $versionInfo');
        });
      }
    }
  } catch (e) {
    safePrint('Error processing ${pubspecFile.path}: $e');
  }
}
